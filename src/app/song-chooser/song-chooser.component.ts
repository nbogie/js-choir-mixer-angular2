...
private songDirsFree: string[] = ["close_to_me", "he_has_done_marvelous_things"];
  songDirs: string[] = ["deep_river", "as", "great_is_thy_faithfulness", "how_great_thou_art",
  ...
  getSongInfos() {
    let allSongInfos = [{ names: this.songDirsFree, root: 'sounds-free/'} , { names: this.songDirs, root: 'sounds/' }]
        .map((obj) => 
            obj.names.map((name) => 
                ({
                    root: obj.root,
